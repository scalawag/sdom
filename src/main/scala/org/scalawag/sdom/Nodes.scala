  ...
  private[this] val implicitNamespaces =
    ...
    if ( ! spec.usesNamespaces ) {
      ...
      val autoGeneratedPrefixes = {
        // Stream.iterate causes an infinite loop here in Scala 2.11
...
//        candidatePrefixes.filterNot(preElementNamespaces.uriToPrefix.contains).iterator
        ...
        val candidatePrefixes = new Iterator[String] {
          var i = 0
          def hasNext = true
          def next(): String = { i += 1; s"ns$i" }
        }
        candidatePrefixes.filterNot(preElementNamespaces.uriToPrefix.contains)
      }
